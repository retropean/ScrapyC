...
#	def __init__(self):
	...
	def parse(self, response):
		...
		for site in sites:
			...
			item['color'] = map(unicode.strip, site.xpath('//section[@class="userbody"]/div[@class="mapAndAttrs"]/p[@class="attrgroup"]/span/text()[2]').extract())
			item['transmission'] = map(unicode.strip, site.xpath('//p[@class="attrgtoup"]/text()[3]').extract())
			item['title'] = map(unicode.strip, site.xpath('//p[@class="attrgtoup"]/span/text()[4]').extract())
